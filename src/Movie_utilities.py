...
def read_movie(line):
    ...
    for num in line[4]:
        if num.isdigit():
            genre_list.append(int(num))
    line[4] = genre_list
    movie = Movie(line[0], int(line[1]), line[2], float(line[3]), line[4])
...
def read_genres():
    ...
    LENGTH = len(Movie.GENRES) - 1
    genres = []
    test = input("Enter a genre number (ENTER to quit): ")
    while test != '' or len(genres) <= 0:
        if not test.isdigit():
            ...
            if test < 0:
                ...
                print("Error: input must be > {}".format(LENGTH))
            else:
                genres.append(test)
        test = input("Enter a genre number (ENTER to quit): ")
    genres.sort()
...
def genre_counts(movies):
    ...
    for movie in movies:
        for genres in movie.genres:
            mother_genre_list.append(genres)
    LENGTH = len(Movie.GENRES)
    for i in range(LENGTH):
        count = mother_genre_list.count(i)
        counts.append(count)
