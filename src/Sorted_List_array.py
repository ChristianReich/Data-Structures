...
class Sorted_List:
        ...
        for i in range(LENGTH):
            if i != 0:
                if self._values[i] == self._values[i - 1]:
                    ind_list.append(i)
        LENGTH2 = len(ind_list)
        for ind in range(-1, -LENGTH2 - 1, -1):
            self._values.pop(ind_list[ind])
    ...
    def count(self, key):
        ...
        if i != -1:
            ...
            number = 0
        while i < self.__len__() and number > 0 and self._values[i] == self._values[i - 1]:
            number += 1
            i += 1
        return number
    ...
    def intersection(self, source1, source2):
        ...
        source1.clean()
        source2.clean()
        source1_len = len(source1)
        source2_len = len(source2)
        i = 0
        if source1._values[i] in source2._values and (not source1._values[i] in val_added):
            val_added.append(source1._values[i])
            self._values.append(source1._values[i])
        while i < (source1_len - 1) or i < (source2_len - 1):
            i += 1
            if source1._values[i] in source2._values and (not source1._values[i] in val_added):
                val_added.append(source1._values[i])
                self._values.append(source1._values[i])
    ...
    def is_identical(self, target):
        ...
        """
        identical = False
        if self.__len__() == len(target) and self.__len__() > 0:
            i = 0
            if self._values[i] != target._values[i]:
                identical = False
            else:
                identical = True
            while identical and i < (self.__len__() - 1):
                i += 1
                if self._values[i] != target._values[i]:
                    identical = False
        elif self.__len__() == len(target) and self.__len__() == 0:
            identical = True
    ...
    def max(self):
        ...
        return deepcopy(self._values[last])
    ...
    def split(self):
        ...
        target2 = Sorted_List()
        half = LENGTH / 2
        if half % 2 != 0 and (half + 1) % 2 != 0:
            half += 0.5
        for i in range(LENGTH):
            if i < half:
                target1.insert(self.remove(self._values[0]))
            else:
                target2.insert(self.remove(self._values[0]))
    ...
    def split_alt(self):
        ...
        for i in range(LENGTH):
            if i % 2 == 0:
                target1.insert(self.remove_front())
            else:
                target2.insert(self.remove_front())
